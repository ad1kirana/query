--select top 3 *
--select hs_cabang,hs_salesoffice,hbrv_nomor_brv,dtt_nomor_terima,hbrv_jenis_brv,hbrv_tgl,
--dtt_nomor_faktur,cust_nama,hs_kode_salesman,
--select top 1 ds_kode_product from meka..tslsd where dtt_nomor_faktur=ds_nomor_faktur) as prod,'', 
...
inner join meka..ttrmtgh on dbrv_nomor_document=htt_nomor_terima
	and hbrv_company=htt_company
	and hbrv_cabang=htt_cabang 
	inner join meka..tcolh on htt_nomor_tagihan=hdt_nomor_tagihan
		and htt_company=hdt_company and htt_cabang=hdt_cabang
			...
			inner join meka..tslsh on dtt_nomor_faktur=hs_nomor_faktur
				...
				and hbrv_cabang=hs_cabang
					inner join meka..mcust on hs_customer=cust_kode
						...
						where hbrv_tgl between '2015-01-01' and '2015-01-31' and hbrv_cabang between '01' and '02'
...
select top 3 * from meka..ttrmtgh
select top 3 * from meka..ttrmtgd
select top 3 * from meka..tbrvd
select top 3 * from meka..tcolh
select top 3 * from meka..tgrh hg_norefference=hdt_nomor_tagihan and 
select top 3 * from meka..ttrmtgd
select top 3 * from meka..ttrmtgh where htt_tanggal between '2015-01-01' and '2015-01-31'
htt_nomor_tagihan=hdt_nomor_tagihan and htt_company=hdt_company and htt_cabang=hdt_cabang