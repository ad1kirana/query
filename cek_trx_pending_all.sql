...
--and hts_no_tran_stock='010117000790BK'
...
select top 10000 hlbm_cabang,hlbm_no_lbm,HRC_No_LBM, hlbm_nomor_dokumen,hlbm_tanggal,'',hlbm_kode_warehous,hlbm_jenis_lbm,hlbm_creator,'',hlbm_kode_customer
from meka..twrsh LEFT join meka..trtrh on
hrc_no_lbm like left(hlbm_no_lbm,14) + '%'
where hlbm_jenis_lbm='rt/m'
and hrc_no_retur is null
and hlbm_tanggal >='2015-01-01'
order by HLBM_Tanggal desc

select top 10 * from MEKA..TRTRH where HRC_No_LBM like '010818000033BM%'

--non_sales_blm_spb
select top 100 hts_cabang,hts_no_tran_stock,hts_nomor_dokumen,hts_tanggal,hts_salesoffice,hts_kode_warehous,hts_jenis_spb,hts_creator,dts_product, dts_kode_barang,dts_qty,'',hts_kode_customer,hts_qq,hts_flag_posting,hts_flag_spb,hts_keterangan
...
--OR pending
select top 1000 * from meka..torderh inner join meka..torderd on hs_nomor_faktur = ds_nomor_faktur 
where hs_flag_posting='n' and hs_flag_spb='n'
--and hs_tanggal between '2015-10-01' and '2015-10-30'
--and ds_kode_barang like 'gt.010'
...
a.hs_kode_salesman,a.hs_creator,c.ds_kode_product,c.ds_kode_barang,c.ds_qty,a.hs_nomor_faktur,d.hlbm_no_lbm
--select top 1000 * 
from meka..tspkm a 
...
where hs_nomor_spk is null and hs_cabang between '04' and '04'  and hs_nomor_vouch=''
and hs_tanggal between '2017-01-01' and '2017-12-31'
--and ds_kode_barang='op-01-sl-oth-017'
--and ds_kode_product <>'oth'
order by hs_nomor_faktur asc 



select top 100 * from meka..tslsh where hs_flag_posting ='n' 
--and hs_cabang='02'
 order by hs_nomor_faktur

--OR tidak ada part-nya
--select top 1000 * from meka..torderh left join meka..torderd on hs_nomor_faktur = ds_nomor_faktur where hs_flag_posting='n' and hs_flag_spb='n' and ds_kode_barang is null order by hs_cabang asc
select top 1000 hs_cabang,hs_nomor_faktur,hs_creator,hs_tanggal,hs_customer,hs_qq from meka..torderh left join meka..torderd on hs_nomor_faktur = ds_nomor_faktur where hs_flag_posting='n' and hs_flag_spb='n' and ds_kode_barang is null order by hs_creator asc

select top 10 * from meka..twrsh where hlbm_no_lbm='010115002862BM'
 

select top 10 * from sjtrade..userlogin where userid='doni'



select top 10 * from meka..torderh where hs_cabang='07' 
select top 10 * from me
--and hs_warehouse='24'
--select top 1000 * from meka..torderh where hs_flag_posting='n' and hs_flag_spb='n' and hs_tanggal < '2011-12-31'-- and '2014-04-15' order by hs_tanggal asc
--select top 1000 * from meka..torderh left join meka..torderd on hs_nomor_faktur = ds_nomor_faktur where hs_nomor_faktur='010409005110OR  '
--update meka..torderh set hs_flag_posting='Y' from meka..torderh where hs_flag_posting='n' and hs_flag_spb='n' and hs_tanggal < '2011-12-31'

--spk_blm_approve
select top 1000 HS_cabang,hs_salesoffice,hs_tanggal,hs_nomor_faktur,hs_warehouse,hs_customer,hs_qq,hs_creator,ds_nomor_faktur, ds_kode_barang,ds_qty_di_order
 from meka..torderh inner join meka..torderd on hs_nomor_faktur = ds_nomor_faktur 
where hs_flag_posting='n' and hs_flag_spb='n'
order by hs_tanggal asc


--OR yg ada part tapi gak ada faktur
--select top 1000 * from meka..torderh inner join meka..torderd on hs_nomor_faktur = ds_nomor_faktur where hs_flag_posting='n' and hs_flag_spb='n' order by hs_tanggal asc
